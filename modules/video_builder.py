...
def _resolve_font(font: str | None) -> str | None:
    ...
    for path in _FONT_CANDIDATES:
        if os.path.exists(path):
            return path
    # Último recurso: tenta "sans-serif" genérico (funciona mas sem acentos)
    return None
...
_IMG_Y = int(H * 0.22)
_TITLE_Y = int(H * 0.14)  # Movido para baixo — agora fica visível (era 0.06)
...
_CAPTION_Y = int(H * 0.38)  # Movido para cima (era 0.42) — mais perto do topo
