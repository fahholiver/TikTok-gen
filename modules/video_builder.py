...
# Caixa do TÍTULO é mais LARGA que a da imagem (mas centralizada no mesmo
# ponto central da imagem). Isso evita que palavras como "Lobisomem" ou
# "Vampiro" sejam cortadas/quebradas no meio por falta de espaço horizontal.
# Se o título tiver duas palavras, ainda pode quebrar linha ENTRE as
# palavras normalmente (isso é ok) — o que não pode é quebrar DENTRO de
# uma palavra.
_TITLE_W = int(_IMG_W * 1.45)
_TITLE_LEFT_X = int(_LEFT_X + _IMG_W / 2 - _TITLE_W / 2)
_TITLE_RIGHT_X = int(_RIGHT_X + _IMG_W / 2 - _TITLE_W / 2)
...
# IMPORTANTE: a legenda é posicionada de forma RELATIVA ao fim das imagens
# (_IMG_Y + _IMG_H), e não como um número fixo. Isso garante que ela NUNCA
# fique em cima das imagens, mesmo que o tamanho delas mude no futuro.
# Testado visualmente: com um valor fixo tipo "0.37" a legenda ainda ficava
# sobrepondo a parte de baixo das imagens (que terminam em ~0.44 de H aqui).
# Por isso calculamos: fim_das_imagens + uma folga de respiro.
_CAPTION_GAP = int(H * 0.025)  # pequena folga de respiro abaixo das imagens
_CAPTION_Y = _IMG_Y + _IMG_H + _CAPTION_GAP
...
def _build_comparison_pair_scenes(data: dict, owl_image_path: str, thinking_path: str,
    ...
    scenes.append(CompositeVideoClip([
        _bg(d2),
        _title_clip(data["item1_title"], _TITLE_LEFT_X, _TITLE_W, _TITLE_Y, d2, font),
        _title_clip(data["item2_title"], _TITLE_RIGHT_X, _TITLE_W, _TITLE_Y, d2, font),
    ...
    scenes.append(CompositeVideoClip([
        ...
        _title_clip(data["item1_title"], _TITLE_LEFT_X, _TITLE_W, _TITLE_Y, d3, font),
        _title_clip(data["item2_title"], _TITLE_RIGHT_X, _TITLE_W, _TITLE_Y, d3, font),
    ...
    scenes.append(CompositeVideoClip([
        ...
        _title_clip(data["item1_title"], _TITLE_LEFT_X, _TITLE_W, _TITLE_Y, d4, font),
        _title_clip(data["item2_title"], _TITLE_RIGHT_X, _TITLE_W, _TITLE_Y, d4, font),
